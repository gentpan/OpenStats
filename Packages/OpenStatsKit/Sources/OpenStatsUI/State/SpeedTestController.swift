...
public final class SpeedTestController {
    ...
    public func runBroadband() {
        ...
        liveBitsPerSecond = 0
        broadbandTask = Task { [limit, route = settings.speedTestRoute] in
    ...
    private func measure<Node: Sendable>(_ nodes: [Node], concurrency: Int,
    ...
    public func download(_ node: GlobalNode) {
        ...
        liveBitsPerSecond = 0
        downloadTask = Task { [limit, route = settings.speedTestRoute] in
            let path = await resolvePath(route)
