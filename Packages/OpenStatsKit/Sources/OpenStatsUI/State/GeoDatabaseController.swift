...
public final class GeoDatabaseController {
    ...
    func update() async {
        ...
        do {
            ...
            message = (changed > 0 ? tr("已更新 \(changed) 个数据库") : tr("已经是最新"), false)
        } catch let failure as Failure {
            message = (failure.text, true)
        } catch {
            message = (tr("更新失败：\(error.localizedDescription)"), true)
    ...
    private func download(_ entry: Manifest.Entry) async throws {
        guard let url = URL(string: entry.file, relativeTo: Self.manifestURL) else { throw Failure(tr("清单里的文件地址无效")) }
        let (temporary, response) = try await URLSession.shared.download(from: url)
        defer { try? FileManager.default.removeItem(at: temporary) }
        guard (response as? HTTPURLResponse)?.statusCode == 200 else { throw Failure(tr("下载 \(entry.edition) 失败")) }
        let digest = try await Task.detached { try Self.sha256(of: temporary) }.value
        guard digest == entry.sha256 else { throw Failure(tr("\(entry.edition) 校验不一致，已丢弃")) }
        let database = try MaxMindDatabase(url: temporary)
        guard database.databaseType == entry.edition else { throw Failure(tr("\(entry.edition) 文件类型不对")) }
    ...
    func importDatabase(from url: URL) {
        do {
            ...
            guard type.hasSuffix("-City") || type.hasSuffix("-Country") || type.hasSuffix("-ASN") else {
                throw Failure(tr("不是城市、国家或 ASN 数据库（\(type)）"))
            }
            ...
            try saveIndex()
            reload()
            message = (tr("已导入 \(type)"), false)
        } catch let failure as Failure {
            message = (failure.text, true)
        } catch {
            message = (tr("导入失败：\(error.localizedDescription)"), true)
        }
    }
