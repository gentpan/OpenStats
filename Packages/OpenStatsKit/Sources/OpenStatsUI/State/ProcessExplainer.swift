...
public final class ProcessExplainer {
    ...
    private func run(facts: String) async {
        ...
        if #available(macOS 26.0, *) {
            ...
            do {
                ...
                guard !Task.isCancelled else { return }
                phase = .failed("Apple 智能没有给出回答：\(error.localizedDescription)")
            }
            return
        }
        #endif
        phase = .failed("需要 macOS 26 及以上，并在系统设置中开启 Apple 智能。")
    }
    ...
    private static func message(for reason: SystemLanguageModel.Availability.UnavailableReason) -> String {
        switch reason {
        case .deviceNotEligible: "这台 Mac 不支持 Apple 智能。"
        case .appleIntelligenceNotEnabled: "请先在“系统设置 → Apple 智能与 Siri”中开启 Apple 智能。"
        case .modelNotReady: "Apple 智能模型还在下载或准备中，请稍后再试。"
        @unknown default: "Apple 智能暂时不可用。"
        }
    }
    #endif
    ...
    nonisolated static func facts(for subject: Subject) -> String {
        var lines = ["进程名：\(subject.name)"]
        if subject.displayName != subject.name { lines.append("显示名称：\(subject.displayName)") }
        let path = subject.executablePath ?? executablePath(pid: subject.pid)
        if let path {
            lines.append("可执行文件：\(path)")
            lines.append("位置：\(location(of: path))")
            if let signer = signer(of: path) { lines.append("签名方：\(signer)") }
        }
        if let bundlePath = subject.appBundlePath, let bundle = Bundle(path: bundlePath) {
            ...
            lines.append("所属应用：\(name)（\(bundle.bundleIdentifier ?? "无 Bundle ID")）")
        }
        if let cpu = subject.cpu { lines.append("CPU：\(Format.percent(cpu))（以单核满载为 100%）") }
        if let memory = subject.memory { lines.append("内存：\(Format.bytes(memory))") }
        if let rate = subject.networkRate { lines.append("网络：\(Format.menuBarRate(rate))") }
    ...
    nonisolated private static func location(of path: String) -> String {
        if path.hasPrefix("/System/") || path.hasPrefix("/usr/") || path.hasPrefix("/sbin/") || path.hasPrefix("/bin/") {
            return "macOS 系统目录（系统组件）"
        }
        if path.hasPrefix("/Applications/") || path.contains("/Applications/") { return "应用程序文件夹" }
        if path.hasPrefix("/Library/") { return "系统级资源库（第三方驱动、辅助工具或后台服务）" }
        if path.contains("/Library/") { return "用户资源库" }
        return "其他位置"
    }
    ...
    nonisolated private static func signer(of path: String) -> String? {
        ...
        if let team = dictionary[kSecCodeInfoTeamIdentifier as String] as? String {
            ...
            return [summary, "团队 \(team)"].compactMap { $0 }.joined(separator: "，")
        }
        return "未签名或临时签名"
    }
