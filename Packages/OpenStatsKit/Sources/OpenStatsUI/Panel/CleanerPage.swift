        ...
        PageScroll {
            ...
            if model.cleaner.needsFullDiskAccess {
                InfoBanner(icon: "lock.shield", text: "部分项目需要“完全磁盘访问权限”才能扫描（Safari 缓存、废纸篓）。", tone: .warning) {
                    Button("去授权") { model.cleaner.openFullDiskAccessSettings() }
...
private struct SummaryCard: View {
    ...
    var body: some View {
        ...
        Card(padding: DS.Space.s3, spacing: DS.Space.s3) {
            HStack(spacing: DS.Space.s1) {
                ...
                Text("磁盘清理").dsFont(.xs, weight: .semibold)
                Spacer()
                if let last = cleaner.lastScan {
                    TimelineView(.periodic(from: .now, by: 60)) { context in
                        Chip(text: "扫描于 " + Self.relative(last, now: context.date))
                    }
                }
            }
            ...
            if let disk = model.store.disk {
                VStack(alignment: .leading, spacing: DS.Space.s1) {
                    ...
                    Text(verbatim: "\(disk.volumeName) 可用 \(Format.bytes(disk.available, base: .decimal)) · 共 \(Format.bytes(disk.total, base: .decimal))")
                        .dsFont(.xs)
            ...
            if cleaner.isConfirming {
                InfoBanner(icon: "exclamationmark.triangle.fill", text: confirmationText, tone: .warning) {
                    HStack(spacing: DS.Space.s2) {
                        Button("取消") { cleaner.cancelClean() }
                            .buttonStyle(DSButtonStyle(kind: .secondary))
                        Button("确认清理") { cleaner.confirmClean() }
                ...
                HStack(spacing: DS.Space.s2) {
                    Button { cleaner.scan() } label: {
                        Label(cleaner.phase == .scanning ? "扫描中…" : "重新扫描", systemImage: "arrow.clockwise")
                    }
                    .buttonStyle(DSButtonStyle(kind: .secondary))
                    .disabled(cleaner.isBusy)

                    Button { cleaner.requestClean() } label: {
                        Label(cleaner.phase == .cleaning ? "清理中…" : "清理所选 \(Format.bytes(cleaner.selectedBytes, base: .decimal))",
                              systemImage: "trash")
                    }
                    .buttonStyle(DSButtonStyle(kind: .primary))
    ...
    private var headline: String {
        ...
        return cleaner.scans.isEmpty ? "扫描中" : Format.bytes(cleaner.totalBytes, base: .decimal)
    }

    private var subline: String {
        let cleaner = model.cleaner
        switch cleaner.phase {
        case .idle, .scanning: return "正在计算可清理空间"
        case .cleaning: return "正在清理…"
        case .ready: return "可清理 · 已选 \(cleaner.selectedScans.count) 项"
        case .finished:
            guard let report = cleaner.report else { return "" }
            var parts = ["已释放"]
            if report.trashedBytes > 0 { parts.append("另有 \(Format.bytes(report.trashedBytes, base: .decimal)) 移到废纸篓") }
            if report.skippedCount > 0 { parts.append("跳过 \(report.skippedCount) 项") }
            if !report.failures.isEmpty { parts.append("\(report.failures.count) 项失败") }
    ...
    private var confirmationText: String {
        ...
        let trashNote = model.settings.cleanPrefersTrash ? "所有内容先移到废纸篓。" : "缓存与日志直接删除，下载内容移到废纸篓。"
        return "将清理 \(count) 个项目，共 \(Format.bytes(cleaner.selectedBytes, base: .decimal))。\(trashNote)"
    }

    private static func relative(_ date: Date, now: Date) -> String {
        let minutes = Int(now.timeIntervalSince(date) / 60)
        return minutes < 1 ? "刚刚" : "\(Format.duration(minutes: minutes))前"
    }
}
    ...
    var body: some View {
        Card(padding: DS.Space.s3, spacing: DS.Space.s2) {
            HStack(spacing: DS.Space.s1) {
                ...
                Text("系统维护").dsFont(.xs, weight: .semibold)
            }
            .foregroundStyle(DS.Palette.textSecondary)

            MaintenanceButton(command: .flushDNS, icon: "network", title: "刷新 DNS 缓存")
            MaintenanceButton(command: .purgeMemory, icon: "memorychip", title: "释放内存")

            Text(model.helper.isReady ? "由辅助工具执行" : "需要管理员授权")
                .dsFont(.xs)
                .foregroundStyle(DS.Palette.textTertiary)
        }
    }
        ...
        Button {
            ...
            VStack(alignment: .leading, spacing: DS.Space.s1 / 2) {
                HStack(spacing: DS.Space.s2) {
                    ...
                    Spacer(minLength: 0)
                    if isRunning {
                        Text("执行中").dsFont(.xs).foregroundStyle(DS.Palette.textTertiary)
                    }
                }
                .foregroundStyle(DS.Palette.textPrimary)
                if let outcome {
...
private struct RuleRow: View {
            ...
            if expanded {
                VStack(alignment: .leading, spacing: DS.Space.s1) {
                    ...
                    if scan.items.count > Self.previewLimit {
                        Text(verbatim: "另有 \(scan.items.count - Self.previewLimit) 项")
                            .dsFont(.xs)
                            .foregroundStyle(DS.Palette.textTertiary)
                    }
                }
    ...
    private var statusText: String {
        ...
        if scan.items.isEmpty { return "无需清理" }
        var text = "\(scan.items.count) 项"
        if scan.skippedCount > 0 { text += " · \(scan.skippedCount) 项使用中" }
        if scan.rule.policy == .trash { text += " · 移到废纸篓" }
        return text
    }
}
...
private struct CleanerFooter: View {
    ...
    var body: some View {
        ...
        HStack(spacing: DS.Space.s3) {
            DSToggle(isOn: $settings.cleanPrefersTrash, label: "缓存也先移到废纸篓")
            VStack(alignment: .leading, spacing: 0) {
                Text("缓存也先移到废纸篓").dsFont(.sm, weight: .medium).foregroundStyle(DS.Palette.textPrimary)
                Text("可以恢复，但清空废纸篓前不会释放空间").dsFont(.xs).foregroundStyle(DS.Palette.textTertiary)
            }
            Spacer()
            Button("查看清理日志") { model.cleaner.revealLog() }
