...
enum MainMenu {
    static func make(target: AnyObject, settingsAction: Selector, updateAction: Selector) -> NSMenu {
        ...
        app.addItem(withTitle: tr("关于 OpenStats"), action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        ...
        app.addItem(.separator())
        app.addItem(withTitle: tr("隐藏 OpenStats"), action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
        app.addItem(.separator())
        app.addItem(withTitle: tr("退出 OpenStats"), action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        add(app, to: main)
